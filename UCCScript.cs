...
namespace UCCScriptRunner
{
    ...
    partial class UCCScript
    {
        ...
        public OrderedDictionary<string, double> GlobalVar = [];

        private static JumpInfo Go = new(Location.Pattern, 0);
        ...
        private struct JumpInfo(Location _location, int _index)
        {
            ...
            public ushort Index = unchecked((ushort)_index);
        }
        ...
        public string Run(string Str)
        {
            ...
            {
                (string p, string r) = Group.First(element => element.Name == pName) switch { UCCScriptInfo ucci => (ucci.Pattern, ucci.Result) };

                string InRes = "";

                Stack<JumpInfo> Location = new();
                bool isJump = false;

                List<string> Pat = [];
                List<ushort> Temporary = [0];
                List<sbyte> Situation = [0];
                List<ushort> deep = [0];

                List<string> formula = [];
                Stack<dynamic> stack = new();
                Stack<bool> Conditions = new();
                Stack<sbyte> ControlType = new();
                Stack<sbyte> ProcessType = new();
                Stack<double> VVar = new();
                ...
                while (true)
                {
                    ...
                    if (Go.Location == UCCScript.Location.Pattern || isJump)
                    {
                        string all_p = Group.First(element => element.Name == pName).Pattern.Trim()[(isJump ? Go.Index : 0)..];

                        isJump = false;
                        Pat = [all_p];

                        int deepCount = deep[^1];
                        int delete = 0;
                        ...
                        while (Pat[^1].Length != 0)
                        {
                            ...
                            switch (Pat[^1][0])
                            {
                                case 'j':
                                    Pat[^1] = Pat[^1][1..];
                                    bool isDQ = false;
                                    int? i = Pat[^1].Zip(Pat[^1].Select((_, i) => i)).Aggregate<(char First, int Second), (char First, int Second)[]>([], (t1, t2) =>
                                    {
                                        if (t2.First == '"' || isDQ)
                                        {
                                            isDQ = !isDQ;
                                        ...
                                        return [.. t1.Concat([t2])];
                                    }).FirstOrDefault(t => t.First == 'j').Second;
                                    int ind = i ?? -1;
                                    Pat[^1] = Pat[^1][(ind + 1)..];
                                    break;
                                case char _ when Pat[^1].StartsWith("m", StringComparison.CurrentCultureIgnoreCase):
                                    Pat[^1] = Pat[^1][1..];
                                    string location = Regex.Match(Pat[^1], @"[0-9]+").Value;
                                    Pat[^1] = Pat[^1][location.Length..];
                                    Location.Push(new(UCCScript.Location.Pattern, p.Length - Pat[^1].Length));
                                    isDQ = false;
                                    Go = new(UCCScript.Location.Result, r.Zip(r.Select((_, i) => i)).Aggregate<(char First, int Second), (char First, int Second)[]>([], (t1, t2) =>
                                    {
                                        if (t2.First == '"') isDQ = !isDQ;
                                        if (isDQ) return t1;
                                        return [.. t1.Concat([t2])];
                                    }).Where(x => x.First == '\'').ElementAt(ushort.Parse(location)).Second);
                                    finish = true;
                                    isJump = true;
                                    continue;
                                case char _ when Pat[^1].StartsWith("ret"):
                                    Pat[^1] = Pat[^1][3..];
                                    Go = Location.Pop();
                                    finish = true;
                                    isJump = true;
                                ...
                                case char _ when Pat[^1].StartsWith("Number"):
                                    s = "";
                                    DeletePat(6);
                                    if (Memory[^1].Success && (new Func<Match, int>(m => new dynamic[] { s = m.Value, m.Index }[1]))(Regex.Match(pStr, @"\d+")) == 0)
                                    {
                                        Delete_pStr(s.Length);
                                        if (deepCount > 0)
                                ...
                                case char _ when Pat[^1].StartsWith("String"):
                                    ...
                                    {
                                        s = "";
                                        if ((new Func<Match, int>(m => new dynamic[] { s = m.Value, m.Index }[1]))(Regex.Match(pStr, @"[^""'\s\d]+")) == 0)
                                        {
                                            Delete_pStr(s.Length);
                    ...
                    if (Memory.Count > 0 && Memory[^1].Success || Go.Location == UCCScript.Location.Result || isJump)
                    {
                        string all_r = Group.First(element => element.Name == pName).Result.Trim()[(isJump ? Go.Index : 0)..];
                        formula = [all_r];

                        isJump = false;
                        int Index() => formula[^1].Select((_, i) => i).First(x => Regex.Replace(formula[^1][..(x + 1)], @"""(\\.|.)*""", "").Count(x => x == ']') - Regex.Replace(formula[^1][..(x + 1)], @"""(\\.|.)*""", "").Count(x => x == '[') == 1);

                        while (formula[^1] != "" && !finish)
                        {
                            switch (formula[^1][0])
                            {
                                case 'j':
                                    ...
                                    bool isDQ = false;
                                    int? i = formula[^1].Zip(formula[^1].Select((_, i) => i)).Aggregate<(char First, int Second), (char First, int Second)[]>([], (t1, t2) =>
                                    {
                                        if (t2.First == '"' || isDQ)
                                        {
                                            isDQ = !isDQ;
                                    ...
                                    int ind = i ?? -1;
                                    formula[^1] = formula[^1][(ind + 1)..];
                                    break;
                                case char _ when formula[^1].StartsWith("Ret"):
                                ...
                                case char _ when formula[^1].StartsWith("Load") && depth > 0 && ControlType.Peek() == 0:
                                    ...
                                    ProcessType.Push(0);
                                    break;
                                case char _ when formula[^1].StartsWith("Move") && depth > 0 && ControlType.Peek() == 0:
                                    ControlType.Push((sbyte)(ControlType.Pop() + 6));
                                    formula[^1] = formula[^1][4..];
                                    ProcessType.Push(20);
                                    ProcessType.Push(0);
                                    break;
                                case char _ when formula[^1].StartsWith("BeMatch") && depth > 0 && ControlType.Peek() == 0:
                                    ControlType.Push((sbyte)(ControlType.Pop() + 8));
                                    formula[^1] = formula[^1][7..];
                                    ProcessType.Push(21);
                                    ProcessType.Push(0);
                                    break;
                                case char _ when formula[^1].StartsWith("GlobalLoad") && depth > 0 && ControlType.Peek() == 0:
                                    ControlType.Push((sbyte)(ControlType.Pop() + 9));
                                    formula[^1] = formula[^1][(10 + formula[^1][10..].TakeWhile(c => c == ' ').Count())..];
                                    LastVName = Regex.Match(formula[^1], @"[^\s[]+").Value;
                                    formula[^1] = formula[^1][LastVName.Length..];
                                    ProcessType.Push(22);
                                    ProcessType.Push(0);
                                ...
                                case ']':
                                    ...
                                    switch (ProcessType.Peek())
                                    {
                                        ...
                                        case 20:
                                            ...
                                            _d = stack.Pop();
                                            index = unchecked((ushort)_d);
                                            Go = new(UCCScript.Location.Pattern, p.Zip(p.Select((_, i) => i)).Where(t => t.First == '\'').ElementAt(index).Second + 1);
                                            Location.Push(new(UCCScript.Location.Result, r.Length - formula[^1].Length + 1));
                                            _ = ProcessType.Pop();
                                            finish = true;
                                            isJump = true;
                                            continue;
                                ...
                                case '~':
                                    ...
                                    break;
                                case '\\' when formula[^1].StartsWith("\\!"):
                                    formula[^1] = formula[^1][2..];
                                    string vName = Regex.Match(formula[^1], @"[^\s]+").Value;
                                    formula[^1] = formula[^1][vName.Length..];
                                    stack.Push(GlobalVar[vName]);
                                    break;
                                case '\\':
                                    formula[^1] = formula[^1][1..];
                                    vName = Regex.Match(formula[^1], @"[^\s]+").Value;
                                    formula[^1] = formula[^1][vName.Length..];
                        ...
                        InRes = string.Join("", stack.Reverse().ToArray());
                    }
                    finish = false;
